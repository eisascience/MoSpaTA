...
Render_GeneExpr_STseqMT1_UMAP <- function(envv, input, GeneSet){
...
Render_GeneExpr_STseqMT1_Spatial <- function(envv, input, GeneSet){
  ...
  if(is.null(envv$MSlideSeqV1CellsWT1) | length(GeneSet) < 1){
    plot(x=0, y=0, main="Load STseq MT1 dataset")
  } else {
    ...
    if(is.null(ranges$GeneExpr_STseqMT1_Spatial_x)){
      gg1
    } else {
      ...
      gg1 + coord_cartesian(xlim = c((xrange[1] + (abs(xrange[1] - xrange[2])*(min(ranges$GeneExpr_STseqMT1_Spatial_x) - 0.2*min(ranges$GeneExpr_STseqMT1_Spatial_x)) )), 
                                     (xrange[1] + (abs(xrange[1] - xrange[2])*(max(ranges$GeneExpr_STseqMT1_Spatial_x) + 0.2*max(ranges$GeneExpr_STseqMT1_Spatial_x)) ))), 
                            ylim = c((yrange[1] + (abs(yrange[1] - yrange[2])*(min(ranges$GeneExpr_STseqMT1_Spatial_x) - 0.2*min(ranges$GeneExpr_STseqMT1_Spatial_x)) )), 
                                     (yrange[1] + (abs(yrange[1] - yrange[2])*(max(ranges$GeneExpr_STseqMT1_Spatial_x) + 0.2*max(ranges$GeneExpr_STseqMT1_Spatial_x)) )) ), expand = T)
...
Render_GeneExpr_SlideSeqV1MT1_UMAP <- function(envv, input, GeneSet){
  
  GeneSet = clean_GeneSet(GeneSet)
  
  
  GeneSet = GeneSet[GeneSet %in% rownames(envv$MSlideSeqV1CellsWT1)]
  ...
  if(is.null(envv$MSlideSeqV1CellsWT1) | length(GeneSet) < 1){
    ...
    if(length(GeneSet)==1) {
      ...
      FeaturePlot(envv$MSlideSeqV1CellsWT1 , reduction = "umap", features = GeneSet[1], 
                  # raster = T, raster.dpi = c(800, 800),
                  order = T, max.cutoff = 'q99', min.cutoff = 'q01')  +
        theme_classic(base_size = 14) +
        theme(axis.line = element_blank(),
      ...
      if(input$GeneExprOpr == "sum"){
        envv$MSlideSeqV1CellsWT1$MultiGeneExpr = colSums(GetAssayData(envv$MSlideSeqV1CellsWT1, assay = "SCT", layer = "data")[GeneSet, ])
        
      } else {
        envv$MSlideSeqV1CellsWT1$MultiGeneExpr = colMeans(GetAssayData(envv$MSlideSeqV1CellsWT1, assay = "SCT", layer = "data")[GeneSet, ])
        
      }
      
      
      FeaturePlot(envv$MSlideSeqV1CellsWT1 , reduction = "umap", features = "MultiGeneExpr",
                  # raster = T, raster.dpi = c(800, 800),
                  order = T, max.cutoff = 'q99', min.cutoff = 'q01')  +
        theme_classic(base_size = 14) +
        theme(axis.line = element_blank(),
...
Render_GeneExpr_SlideSeqV1MT1_Spatial <- function(envv, input, GeneSet){
  ...
  if(is.null(envv$MSlideSeqV1CellsWT1) | length(GeneSet) < 1){
    ...
    gg1=  SpatialFeaturePlot(envv$MSlideSeqV1CellsWT1 , features = GeneSet[1], 
                         # raster = T, raster.dpi = c(800, 800),
                         max.cutoff = 'q99', min.cutoff = 'q01')  +
        theme_classic(base_size = 14) +
        theme(axis.line = element_blank(),
     ...
     gg1= SpatialFeaturePlot(envv$MSlideSeqV1CellsWT1 , features = "MultiGeneExpr",
                         # raster = T, raster.dpi = c(800, 800),
                         max.cutoff = 'q99', min.cutoff = 'q01')  +
        theme_classic(base_size = 14) +
        theme(axis.line = element_blank(),
    ...
    if(is.null(ranges$GeneExpr_SlideSeqV1MT1_Spatial_x)){
      gg1
    } else {
      xrange = ggplot_build(gg1)$layout$panel_params[[1]]$x.range
      yrange = ggplot_build(gg1)$layout$panel_params[[1]]$y.range
      ...
      gg1 + coord_cartesian(xlim = c((xrange[1] + (abs(xrange[1] - xrange[2])*(min(ranges$GeneExpr_SlideSeqV1MT1_Spatial_x) - 0.2*min(ranges$GeneExpr_SlideSeqV1MT1_Spatial_x)) )), 
                                     (xrange[1] + (abs(xrange[1] - xrange[2])*(max(ranges$GeneExpr_SlideSeqV1MT1_Spatial_x) + 0.2*max(ranges$GeneExpr_SlideSeqV1MT1_Spatial_x)) ))), 
                            ylim = c((yrange[1] + (abs(yrange[1] - yrange[2])*(min(ranges$GeneExpr_SlideSeqV1MT1_Spatial_x) - 0.2*min(ranges$GeneExpr_SlideSeqV1MT1_Spatial_x)) )), 
                                     (yrange[1] + (abs(yrange[1] - yrange[2])*(max(ranges$GeneExpr_SlideSeqV1MT1_Spatial_x) + 0.2*max(ranges$GeneExpr_SlideSeqV1MT1_Spatial_x)) )) ), expand = T)
...
Threshold_GeneExpr_SlideSeqV1MT1_Spatial <- function(envv, input, GeneSet){
  ...
  if(is.null(envv$MSlideSeqV1CellsWT1) | length(GeneSet) < 1){
    ...
    if(length(GeneSet)==1) {
      ...
      envv$MSlideSeqV1CellsWT1$GeneExpr = colSums(GetAssayData(envv$MSlideSeqV1CellsWT1, assay = "SCT", layer = "data")[GeneSet, ])
      ...
      if(input$GeneExprOpr == "sum"){
        ...
        envv$MSlideSeqV1CellsWT1$GeneExpr = colMeans(GetAssayData(envv$MSlideSeqV1CellsWT1, assay = "SCT", layer = "data")[GeneSet, ])
    ...
    tempExprVec = envv$MSlideSeqV1CellsWT1$GeneExpr
    
    envv$MSlideSeqV1CellsWT1$ExprBL = (ifelse( tempExprVec > max( tempExprVec ) * input$ExprThresh,
                                               "Above", "Below"))
...
Threshold_GeneExpr_STseqMT1_Spatial<- function(envv, input, GeneSet){
...
Render_GeneExpr_SlideSeqV1MT2_UMAP <- function(envv, input, GeneSet){
  
  GeneSet = clean_GeneSet(GeneSet)
  
  
  GeneSet = GeneSet[GeneSet %in% rownames(envv$MSlideSeqV1CellsWT2)]
  
  print(GeneSet)
  
  if(is.null(envv$MSlideSeqV1CellsWT2) | length(GeneSet) < 1){
    plot(x=0, y=0, main="Load STseq MT2 dataset")
  } else {
    
    if(length(GeneSet)==1) {
      
      FeaturePlot(envv$MSlideSeqV1CellsWT2 , reduction = "umap", features = GeneSet[1], 
                  # raster = T, raster.dpi = c(800, 800),
                  order = T, max.cutoff = 'q99', min.cutoff = 'q01')  +
        theme_classic(base_size = 14) +
        theme(axis.line = element_blank(),
      ...
      if(input$GeneExprOpr == "sum"){
        envv$MSlideSeqV1CellsWT2$MultiGeneExpr = colSums(GetAssayData(envv$MSlideSeqV1CellsWT2, assay = "SCT", layer = "data")[GeneSet, ])
        
      } else {
        envv$MSlideSeqV1CellsWT2$MultiGeneExpr = colMeans(GetAssayData(envv$MSlideSeqV1CellsWT2, assay = "SCT", layer = "data")[GeneSet, ])
        
      }
      
      
      FeaturePlot(envv$MSlideSeqV1CellsWT2 , reduction = "umap", features = "MultiGeneExpr",
                  # raster = T, raster.dpi = c(800, 800),
                  order = T, max.cutoff = 'q99', min.cutoff = 'q01')  +
        theme_classic(base_size = 14) +
        theme(axis.line = element_blank(),
...
Render_GeneExpr_SlideSeqV1MT2_Spatial <- function(envv, input, GeneSet){
  ...
  if(is.null(envv$MSlideSeqV1CellsWT2) | length(GeneSet) < 1){
    ...
    if(length(GeneSet)==1) {
      ...
      gg1=  SpatialFeaturePlot(envv$MSlideSeqV1CellsWT2 , features = GeneSet[1], 
                               # raster = T, raster.dpi = c(800, 800),
                               max.cutoff = 'q99', min.cutoff = 'q01')  +
        theme_classic(base_size = 14) +
        theme(axis.line = element_blank(),
      ...
      gg1= SpatialFeaturePlot(envv$MSlideSeqV1CellsWT2 , features = "MultiGeneExpr",
                              # raster = T, raster.dpi = c(800, 800),
                              max.cutoff = 'q99', min.cutoff = 'q01')  +
        theme_classic(base_size = 14) +
        theme(axis.line = element_blank(),
    ...
    if(is.null(ranges$GeneExpr_SlideSeqV1MT2_Spatial_x)){
      gg1
    } else {
      xrange = ggplot_build(gg1)$layout$panel_params[[1]]$x.range
      yrange = ggplot_build(gg1)$layout$panel_params[[1]]$y.range
      ...
      gg1 + coord_cartesian(xlim = c((xrange[1] + (abs(xrange[1] - xrange[2])*(min(ranges$GeneExpr_SlideSeqV1MT2_Spatial_x) - 0.2*min(ranges$GeneExpr_SlideSeqV1MT2_Spatial_x)) )), 
                                     (xrange[1] + (abs(xrange[1] - xrange[2])*(max(ranges$GeneExpr_SlideSeqV1MT2_Spatial_x) + 0.2*max(ranges$GeneExpr_SlideSeqV1MT2_Spatial_x)) ))), 
                            ylim = c((yrange[1] + (abs(yrange[1] - yrange[2])*(min(ranges$GeneExpr_SlideSeqV1MT2_Spatial_x) - 0.2*min(ranges$GeneExpr_SlideSeqV1MT2_Spatial_x)) )), 
                                     (yrange[1] + (abs(yrange[1] - yrange[2])*(max(ranges$GeneExpr_SlideSeqV1MT2_Spatial_x) + 0.2*max(ranges$GeneExpr_SlideSeqV1MT2_Spatial_x)) )) ), expand = T)
...
Threshold_GeneExpr_SlideSeqV1MT2_Spatial <- function(envv, input, GeneSet){
  ...
  if(is.null(envv$MSlideSeqV1CellsWT2) | length(GeneSet) < 1){
    ...
    if(length(GeneSet)==1) {
      ...
      envv$MSlideSeqV1CellsWT2$GeneExpr = colSums(GetAssayData(envv$MSlideSeqV1CellsWT2, assay = "SCT", layer = "data")[GeneSet, ])
      ...
      if(input$GeneExprOpr == "sum"){
        ...
        envv$MSlideSeqV1CellsWT2$GeneExpr = colMeans(GetAssayData(envv$MSlideSeqV1CellsWT2, assay = "SCT", layer = "data")[GeneSet, ])
    ...
    tempExprVec = envv$MSlideSeqV1CellsWT2$GeneExpr
    
    envv$MSlideSeqV1CellsWT2$ExprBL = (ifelse( tempExprVec > max( tempExprVec ) * input$ExprThresh,
                                               "Above", "Below"))
...
Threshold_GeneExpr_STseqMT2_Spatial<- function(envv, input, GeneSet){
