...
#' @param input environment list associated with innput paramters in ShinySDA
#' @return the updated environment list
Load_SlideSeqMT2 <- function(envv){
  
  
  if(is.null(envv$MSlideSeqV1CellsWT2)) {
    
    envv$MSlideSeqV1CellsWT2 = readRDS( "./data/WT2_serSCTbase_v2_SerRedObj.rds")
    
  } else {
    print("SlideSeqV1 MT2 already leaded ")
  }
